...
def auth_host_without_business(request):
    ...
    return render(request,'assets/auth_type.html', locals())
...
def update_cabinet(request):
    if request.method == 'POST':
        ...
        uf = server_room_hous()
        return render(request,'assets/update_cabinet.html', locals())
...
def update_system(request):
    if request.method == 'POST':
        ...
        return render(request,'assets/update_system.html', locals())
...
def select_business(request):
    if request.method == 'POST':
        ...
        node_name = request.GET.get('node_name', '')
        return render(request,'assets/select_business.html', locals())
...
def Index_add_batch(request):
    ...
    if request.method == 'POST':
        ...
        for i in batch_ok:
            print
            i[0], str(i[1]).replace(':', '-'), i[2], i[3], i[4], i[5]
            mac = str(i[1]).replace(':', '-')
        ...
        return render(request,'assets/add_batch.html', content)
    else:
        content.update(csrf(request))
        content["batch"] = True
        return render(request,'assets/add_batch.html', content)
...
def services_list_id(request, uuid):
    ...
    return render(request,'assets/ajax_item.html', locals())
...
def services_room_id(request):
    ...
    if request.method == 'POST':
        ...
        return render(request,'assets/ajax_item.html', locals(), context_instance = RequestContext(request, ))
...
def Node_search(request):
    ...
    try:
        ...
        # return render(request,'assets/host_list.html', locals(), context_instance=RequestContext(request))
...
def approve_list(request):
    ...
    server_type = Project.objects.all()
    return render(request,'assets/approve_vm.html', locals())
...
def node_update(request):
    ...
    try:
        ...
        if node:
            ...
            node.save()
            return HttpResponse(
                json.dumps({"status": "200", "result": "save is ok"}, ensure_ascii = False, indent = 4, ))
    except Host.DoesNotExist:
        return HttpResponse(
            json.dumps({"status": "404", "result": "over", "data": eth1}, ensure_ascii = False, indent = 4, ))
...
def server_order_by(request):
    ...
    # return render(request,'assets/approve_vm.html', locals(), context_instance=RequestContext(request))
